...
pair_lmm <- as.data.frame(pairs(emmeans(claydye_pol, ~ treatment | time)))
pair_lmm$experiments <- "claydye"
pair_lmm$dependent_variable <- "polarity"
pair_lmm <- pair_lmm %>%
            select(experiments, dependent_variable, everything())

#exporting the results to .csv
write_csv(results_lmm_coll, "full_comparison_proximity_LMMs.csv")
write_csv(pair_lmm, "full_comparison_pairwise_polarity_LMM.csv")
```