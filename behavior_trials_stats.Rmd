...
trans_shelter$sheltered_time_s <- log(trans_shelter$sheltered_time_s+1)
...
color_over <- glmmTMB(swim_over ~ color*treatment*recording + (1|tankID),
                      data = color_data)
...
#post hoc pairwise comparisons of the significant results
color_pairs <- list()
color_pairs[[1]] <- as.data.frame(pairs(emmeans(pois_color_over, ~ color)))
color_pairs[[2]] <- as.data.frame(pairs(emmeans(pois_color_border, ~ color)))
inter_pairs <- list()
