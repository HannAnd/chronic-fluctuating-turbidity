...
plot(d_con_cd_cont) #assumptions borderline
plot(d_con_cd_foll) #assumptions borderline
plot(d_con_cd_angr) #assumptions borderline
plot(d_con_cd_all) #assumptions borderline

#transforming the data to meet model assumptions
direct_cd_trans <- direct_claydye
for (i in 1:6) {
  direct_cd_trans[[i]]$behav_count <- log(direct_cd_trans[[i]]$behav_count+1)
}
...
d_con_cd_cont_trans <- update(d_con_cd_cont, . ~ .,  data = direct_cd_trans[[3]])
d_con_cd_foll_trans <- update(d_con_cd_foll, . ~ .,
                               data = direct_cd_trans[[4]])
d_con_cd_angr_trans <- update(d_con_cd_angr, . ~ .,
                              data = direct_cd_trans[[5]])
d_con_cd_all_trans <- update(d_con_cd_all, . ~ ., data = direct_cd_trans[[6]])

#checking the transformed model assumptions
plot(d_con_cd_chase_trans) #assumptions worse
plot(d_con_cd_lat_trans) #assumptions borderline
plot(d_con_cd_cont_trans) #assumptions worse
plot(d_con_cd_foll_trans) #assumptions worse
plot(d_con_cd_angr_trans) #assumptions worse
plot(d_con_cd_all_trans) #assumptions worse
```
