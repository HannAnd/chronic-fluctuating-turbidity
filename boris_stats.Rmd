...
pair_lmm <- list()
pair_lmm[[1]] <- as.data.frame(pairs(emmeans(f_dur_dd_angr_trans, ~ treatment |
                                                                    time)))
pair_lmm[[2]] <- as.data.frame(pairs(emmeans(f_dur_dd_all, ~ treatment)))

for (i in 1:2) {
  pair_lmm[[i]]$experiments <- "dyedye"
}
pair_lmm[[1]]$dependent_variable <- "aggregated_aggression"
pair_lmm[[2]]$dependent_variable <- "all_behaviors"
for (i in 1:2) {
...
write_csv(pair_lmm[[1]], "full_pairwise_aggregated_aggression_duration_LMMs.csv")
