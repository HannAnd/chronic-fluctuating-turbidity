...
knitr::opts_knit$set(root.dir = "D://2 Day Mini Kaolin Turb Flux Experiment/datafiles_processed")
library(tidyverse)
library(data.table)
library(zoo)
...
for (k in 1:(length(list_csv)/fish)) {  #across the whole file list
  ...
  for (z in 1:fish) {  #across one video of files
    ...
    simpspeed <- data.frame(matrix(nrow = floor(nrow(dista)/30), ncol = 2))
    ...
    rollspeed <- data.frame(matrix(nrow = nrow(simpspeed), ncol = 3))
  ...
  finalframe$mean_speed[k] <- mean(rollspeed$speed, na.rm = TRUE)
  finalframe$activity_ratio[k] <- (sum(rollspeed$speed >= 10, na.rm = TRUE)/
                                    sum(rollspeed < 10, na.rm = TRUE)) #these are place-holder values
  finalframe$speed_variance[k] <- var(rollspeed$speed, na.rm = TRUE)
  finalframe$max_speed[k] <- max(rollspeed$speed, na.rm = TRUE)
  finalframe$max_acceleration[k] <- max(rollspeed$acceleration, na.rm = TRUE)
  
  i = i+fish
}

write_csv(finalframe, "D://2 Day Mini Kaolin Turb Flux Experiment/2day_clay_movement_data.csv")
```
