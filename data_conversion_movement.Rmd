...
for (k in 1:(length(list_csv)/fish)) {  #across the whole file list
  ...
  finalframe$mean_speed[k] <- mean(unlist(shoalspeed, use.names = FALSE),
                                   na.rm = TRUE)
  finalframe$activity_ratio[k] <- (sum(unlist(shoalspeed, use.names = FALSE) >= 10,
                                       na.rm = TRUE)/
                                    sum(unlist(shoalspeed, use.names = FALSE) < 10,
                                        na.rm = TRUE)) #these are place-holder values
  ...
  finalframe$max_speed[k] <- max(unlist(shoalspeed, use.names = FALSE),
                                 na.rm = TRUE)
  finalframe$max_acceleration[k] <- max(unlist(shoalaccel, use.names = FALSE),
                                        na.rm = TRUE)
  
  i = i+fish
}

write_csv(finalframe, "E://2 Week Kaolin Turb Flux Experiment/2week_clay_movement_data.csv")
```
