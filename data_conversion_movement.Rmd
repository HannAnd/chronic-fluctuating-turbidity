...
knitr::opts_knit$set(root.dir = "D://2 Week Kaolin Turb Flux Experiment/test")
library(tidyverse)
library(data.table)
library(zoo)
...
for (k in 1:(length(list_csv)/fish)) {  #across the whole file list
  ...
  finalframe$distance_cm[k] <- sum(unlist(shoalspeed, use.names = FALSE),
                                   na.rm = TRUE)
  finalframe$mean_speed[k] <- mean(unlist(shoalspeed, use.names = FALSE),
                                   na.rm = TRUE)
  #ratio of active to inactive windows
